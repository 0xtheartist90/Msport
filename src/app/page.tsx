...
import { rangeHighlights, ballRates } from '@/lib/site-content';
import { getLocalizedNewsItems } from '@/lib/localized-content';

const [, bayLayoutFeature, tourBallFeature] = rangeHighlights;
...
export default function HomePage() {
  ...
  const [membershipMobileSlide, setMembershipMobileSlide] = useState(0);
  const [newsMobileSlide, setNewsMobileSlide] = useState(0);
  const membershipCarouselRef = useRef<HTMLDivElement>(null);
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between relative">
      ...
      <section id="range" className="section-anchor section-matcha py-16 md:py-24 w-full">
            ...
            {localizedRangeCards.map((card, index) => {
              ...
              return (
                <Link
                  key={card.key}
                  href={card.href}
                  className={`group block h-full rounded-[26px] focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40 reveal reveal-up ${delayClass}`}
                >
                  <article className="relative flex h-full min-h-[320px] flex-col justify-between overflow-hidden rounded-[26px] border border-white/10 p-6 text-white shadow-[0_30px_80px_rgba(0,0,0,0.45)] transition-all duration-500 ease-out transform-gpu group-hover:-translate-y-3 group-hover:rotate-1 group-hover:shadow-[0_42px_110px_rgba(0,0,0,0.52)] sm:min-h-[300px] sm:p-8">
                    ...
                    <div className="relative z-10 min-h-[11.5rem] space-y-3 transition-transform duration-500 group-hover:-translate-y-1 sm:min-h-[8.5rem]">
                      <p className="text-[1.85rem] font-black tracking-tight sm:text-3xl md:text-4xl">{card.copy.title}</p>
                      {card.copy.subtitle && (
                        <p className="max-w-[30rem] text-xs text-white/75 transition-colors duration-300 group-hover:text-white/88 sm:text-sm">
                          {card.copy.subtitle}
                        </p>
                      )}
      ...
      <section id="membership" className="relative section-anchor section-smoke py-16 md:py-24 w-full overflow-hidden">
        <div className="relative mx-auto w-full max-w-6xl px-6 lg:px-8 space-y-8 md:space-y-12">
          <SectionHeader
          ...
          <div className="md:hidden space-y-4 reveal reveal-up reveal-delay-1">
            <div
              ref={membershipCarouselRef}
              onScroll={event => {
                const { scrollLeft, clientWidth } = event.currentTarget;
                const nextSlide = Math.round(scrollLeft / clientWidth);
                if (nextSlide !== membershipMobileSlide) {
                  setMembershipMobileSlide(nextSlide);
                }
              }}
            ...
            >
                    ...
                    <div className="relative z-10 p-8 text-white">
                      <div className="flex flex-col gap-6">
                        <div className="max-w-2xl space-y-4">
                          ...
                          <h3 className="text-3xl font-black leading-tight">{t.membership.rangeSelection.title}</h3>
                        ...
                        <div className="w-full rounded-[28px] border border-white/10 bg-white/5 p-6 backdrop-blur-xl">
                          <p className="label-light text-center">{t.membership.ratesLabel}</p>
                              ...
                              return (
                                <div key={rate.type} className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3">
                                  <div className="flex items-start justify-between gap-3">
                                    <p className="text-[0.78rem] font-semibold uppercase tracking-[0.14em] text-white/75">
                                      {trayLabel}
                                    </p>
                                    <p className="text-[0.52rem] font-semibold uppercase tracking-[0.14em] text-white/55">
                                      {copy.standardRate}
                                    </p>
                                  </div>
                                  <p className="text-[1.65rem] font-black tracking-tight">
                                    {`฿${price.toLocaleString(locale)} ${copy.trayRateSuffix}`}
                                  </p>
                                  <p className="max-w-[26ch] text-[0.68rem] leading-snug text-white/60">
                ...
                {localizedMembershipCards.map(card => {
                  return (
                    <div key={card.translationKey} className="flex w-full flex-shrink-0 snap-center pr-4 last:pr-0">
                      <article className="flex h-full w-full flex-col overflow-hidden rounded-[30px] border border-white/10 bg-[#121210] shadow-none">
                          ...
                          <div className="relative h-full min-h-[260px]">
                            ...
                            <div className="absolute inset-0 flex flex-col justify-between p-5">
                              <div className="flex items-start justify-between gap-4">
                                ...
                                <span className="rounded-full border border-white/20 bg-black/20 px-2.5 py-1 text-[0.5rem] font-semibold uppercase tracking-[0.2em] text-white/78 backdrop-blur-md">
                        ...
                        <div className="relative flex-1 border-t border-white/10">
                          <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(10,10,10,0.18),rgba(255,255,255,0.05))] backdrop-blur-md" />
                          <div className="relative flex h-full flex-col space-y-4 p-5">
                            <div className="flex items-start justify-between gap-3">
                              <p className="max-w-[16rem] text-[0.56rem] font-semibold uppercase tracking-[0.18em] text-white/55">
                                {card.copy.membershipTag}
                              </p>
                              <p className="shrink-0 text-[0.52rem] font-semibold uppercase tracking-[0.18em] text-white/52">
                                {copy.bestValue}
                              </p>
                            </div>
                                ...
                                {card.packages.map((pkg, idx) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`rounded-[22px] border px-3 py-4 text-left backdrop-blur transition-all duration-300 ${
                                        isSelected
                                          ? 'border-[var(--accent)] bg-[linear-gradient(180deg,rgba(185,28,28,0.18),rgba(255,255,255,0.08))]'
                                    ...
                                    >
                                      <div className="space-y-4">
                                        <div className="space-y-1 text-center">
                                          {trayCount ? (
                                            <p className="text-[0.52rem] font-semibold uppercase tracking-[0.12em] text-white/78">
                                              {trayCount} {copy.trays}
                                            </p>
                                          ) : null}
                                          <p className="whitespace-nowrap text-[1rem] font-black tracking-tight text-white">
                                            {`฿${trayRate.toLocaleString(locale)} ${copy.trayRateSuffix}`}
                                          </p>
                                        </div>
                                        <div className="rounded-[16px] border border-white/10 bg-black/12 px-2 py-3 text-center">
                                          {isSelected && freeTrays ? (
                                            <p className="text-[0.48rem] font-semibold uppercase tracking-[0.1em] text-[var(--accent)]">
                                              {freeTrays} {copy.freeTrays}
                                            </p>
                                          ) : null}
                                          <p className="mt-2 text-[0.45rem] font-semibold uppercase tracking-[0.12em] text-white/45">
                                            {copy.total}
                                          </p>
                                          <p className={`mt-1 text-[0.82rem] font-black tracking-tight ${isSelected ? 'text-[var(--accent)]' : 'text-white/72'}`}>
            ...
            <div className="flex items-center justify-center gap-2">
              {[0, 1, 2].map(index => {
                const isSelected = membershipMobileSlide === index;

                return (
                  <button
                    key={`membership-slide-${index}`}
                    type="button"
                    aria-label={`Show membership slide ${index + 1}`}
                    onClick={() => {
                      setMembershipMobileSlide(index);
                      membershipCarouselRef.current?.scrollTo({
                        left: membershipCarouselRef.current.clientWidth * index,
                        behavior: 'smooth'
                      });
                    }}
                    className={`h-2.5 rounded-full transition-all duration-300 ${
                      isSelected ? 'w-7 bg-[var(--accent)]' : 'w-2.5 bg-white/30'
                    }`}
                  />
                );
              })}
                          ...
                          <div className="md:hidden">
                            {card.packages.map((pkg, idx) => {
                              ...
                              return (
                                <button
                                  ...
                                  className="w-full rounded-[22px] border border-[var(--accent)] bg-[linear-gradient(180deg,rgba(185,28,28,0.18),rgba(255,255,255,0.08))] px-4 py-4 text-left shadow-[0_18px_38px_rgba(0,0,0,0.22)] backdrop-blur transition-all duration-300"
                                >
                                  <div className="space-y-4">
                                    <div className="space-y-1 text-center">
                                      {trayCount ? (
                                        <p className="text-[0.58rem] font-semibold uppercase tracking-[0.14em] text-white/78">
                                          {trayCount} {copy.trays}
                                        </p>
                                      ) : null}
                                      <p className="whitespace-nowrap text-[1.3rem] font-black tracking-tight text-white">
                                        {`฿${trayRate.toLocaleString(locale)} ${copy.trayRateSuffix}`}
                                      </p>
                                    </div>
                                    <div className="rounded-[16px] border border-white/10 bg-black/12 px-3 py-3 text-center">
                                      {freeTrays ? (
                                        <p className="text-[0.52rem] font-semibold uppercase tracking-[0.12em] text-[var(--accent)]">
                                          {freeTrays} {copy.freeTrays}
                                        </p>
                                      ) : null}
                                      <p className="mt-2 text-[0.48rem] font-semibold uppercase tracking-[0.14em] text-white/45">
                                        {copy.total}
                                      </p>
                                      <p className="mt-1 text-[0.95rem] font-black tracking-tight text-[var(--accent)]">
                                        {`฿${packageAmount.toLocaleString(locale)}`}
                                      </p>
                                    </div>
                                  </div>
                            ...
                            <div className="mt-4 flex items-center justify-center gap-2">
                              {card.packages.map((pkg, idx) => {
                                ...
                                return (
                                  <button
                                    key={`${card.translationKey}-dot-${pkg.package}`}
                                    type="button"
      ...
      <section className="section-anchor section-matcha py-16 md:py-24 w-full border-t border-white/10">
        <div className="mx-auto w-full max-w-7xl px-6 lg:px-8 space-y-6 md:space-y-8">
      ...
      <section id="news" className="section-anchor section-cream py-16 md:py-24 w-full border-t border-[#706C61]/10">
        <div className="mx-auto w-full max-w-6xl px-6 lg:px-8 space-y-8 md:space-y-10">
          <SectionHeader label={t.news.section.label} title={copy.latestNewsTitle} subtitle={t.news.section.subtitle} />

          <div
            ref={newsCarouselRef}
            className="flex snap-x snap-mandatory gap-5 overflow-x-auto overflow-y-hidden pb-2 touch-pan-x [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:hidden"
            onScroll={event => {
              ...
              setNewsMobileSlide(Math.round(container.scrollLeft / slideWidth));
            }}
          >
            {localizedNewsItems.slice(0, 3).map((item, index) => {
              ...
              return (
                <Link
                  key={item.slug}
                  href={`/news/${item.slug}`}
                  className={`group block h-full min-w-full snap-start rounded-[12px] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1C1C1A]/20 reveal reveal-up ${delayClass} md:min-w-0`}
                >
                  <article className="flex h-full min-h-[26rem] flex-col overflow-hidden rounded-[12px] border border-[#DCD8CF] bg-white shadow-none transition-all duration-500 ease-out transform-gpu group-hover:-translate-y-3 group-hover:rotate-1 md:min-h-0 md:shadow-[0_12px_35px_rgba(0,0,0,0.08)] md:group-hover:shadow-[0_24px_58px_rgba(0,0,0,0.14)]">
                    ...
                    <div className="flex flex-1 flex-col gap-4 p-6 text-left transition-transform duration-500 group-hover:-translate-y-1">
                      <div className="flex min-h-[1.25rem] items-center justify-between text-[0.58rem] uppercase tracking-[0.4em]">
                        <span className="text-[#8f8f8f]">{item.badge}</span>
                        <span className="text-[var(--accent)]">{item.date}</span>
                      </div>
                      <h3 className="min-h-[6.5rem] text-[1.35rem] font-black leading-[1.2] text-[#151515]">
                        {item.title}
                      </h3>
          ...
          <div className="flex items-center justify-center gap-2 md:hidden">
            {localizedNewsItems.slice(0, 3).map((item, index) => {
              const isActive = newsMobileSlide === index;
              ...
              return (
                <button
                  ...
                  className={`h-2.5 rounded-full transition-all duration-300 ${
                    isActive ? 'w-8 bg-[var(--accent)]' : 'w-2.5 bg-[#C5C0B5]'
                  }`}
                />
              );
            })}
